(() => {
  ...
  const BACKEND_BASE = "https://forest-backend-ccf7.onrender.com";
  ...
  function showLogin() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (msg) {
          msg.textContent = (err && (err.error || err.message)) || "登录失败，请检查网络或稍后再试";
  ...
  function showRegister() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (msg) {
          msg.textContent = (err && (err.error || err.message)) || "注册失败，请检查网络或稍后再试";
          msg.className = "auth-msg err";
        }
      } finally {
